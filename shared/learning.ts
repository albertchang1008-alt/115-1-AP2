export interface LearningEvent {
  id: string;
  type: 'explore' | 'answer' | 'hint' | 'time' | 'node_time' | 'completed';
  nodeId?: string;
  questionId?: string;
  correct?: boolean;
  seconds?: number;
...
export const emptyLearning = (): LearningSummary => ({ nodes: [], answers: {}, hints: 0, activeSeconds: 0, nodeSeconds: {}, completed: false });
export function validEvent(e: LearningEvent) {
  return !!e && typeof e.id === 'string' && /^[\w-]{1,100}$/.test(e.id) && (
    ...
    (e.type === 'node_time' && typeof e.nodeId === 'string' && /^[\w-]{1,100}$/.test(e.nodeId) && Number.isFinite(e.seconds) && e.seconds! > 0 && e.seconds! <= 60)
  );
}
export function reduceLearning(old: LearningSummary, events: LearningEvent[]): LearningSummary {
  ...
  for (const e of events) {
    ...
    if (e.type === 'node_time') result.nodeSeconds[e.nodeId!] = (result.nodeSeconds[e.nodeId!] || 0) + e.seconds!;
