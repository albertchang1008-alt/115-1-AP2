...
這是「課程平台1.0」（115-1 AP2 課程平台）的程式碼庫。這份文件跟 `CLAUDE.md`
內容相同，只是給非 Claude 的 AI coding agent（會找 AGENTS.md 而不是
CLAUDE.md 的工具）看。開始任何工作之前：
...
3. **完成一段工作之後（不論是回答問題、改程式、寫文件），必須更新
   ...
   下一個接手的 agent（不限哪個 AI 工具）會從過期資訊開始工作。
4. 版本異動歷史記在 `DEVELOPMENT_LOG.md`（新版本寫最上面），跟 `handoff.md`
   分工不同：`DEVELOPMENT_LOG.md` 是流水帳，`handoff.md` 是「現在的狀態」。
5. Commit message 結尾附上身分標註，方便回溯是哪個 agent、哪次做的改動。
   不要用 `--no-verify`、不要強制推送、不要略過測試。
