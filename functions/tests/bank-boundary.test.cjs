...
test('題庫發布與 500 題交卷串接：留白題序、重送去重、超量拒絕', async () => {
  ...
  try {
    ...
    const attempt = { id:'attempt1', courseId:'ap2', unitId:'unit01', version:bank.version, mode:'quiz', full:true, score:0, clientAt:Date.now(), duration:500, answers: questions.map((q) => ({ questionId:q.id, selected:'a', correct:true, seconds:1 })) };
