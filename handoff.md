...
38.（2026-09-16，僅規畫、未實作）止血教材測驗擬改為兩層：第一層「先備知識建構」用 6 題依流程圖排列的低門檻辨識／順序／名詞－功能題，錯後立即以圖解回饋並可重答，全部掌握後解鎖；第二層保留 5 題隨機的實務病例／易混淆應用題，病例層全對才呼叫教材 `complete()`。題目 ID 應以 `hemostasis-foundation-q01...` 與 `hemostasis-case-q01...` 分組，畫面分別顯示兩層進度。現有平台 `answer(questionId, correct)` 能保存 11 題作答事件，但教師通用診斷目前只看總題數；正式實作時需把活動題目分母從 5 改 11，並擴充診斷頁依 ID 前綴分列基礎層與病例層首次正確／重試，避免混成一個分數。不可用重試數或時間直接判定學生是否認真。
