...
目前版本：1.3.0。2026-09-18 已完成學生端 UI 改版 1.3.0：新增 `Unit.visibility`／歷史標籤／學生提示與教師「學習進度」看板；以四個專用 Callable Functions transaction 同步 draft＋published 的區域欄位，絕不把其他草稿或題庫版本推到 published。`hidden` 由 `forClass()` 與 getProgress/getHistory 後端過濾，Firestore rules 維持全面拒絕客戶端存取。完成度使用第 2 版規則（達標＋互動教材＋外部連結），快照記錄規則版本、舊快照缺欄位視為第 1 版；`submitAttempt` 現在以後端題庫快照重批，忽略前端 score/correct。SDK 不再發送整數化後為 0 的 node_time。`npm run check` 已通過（前端 39、後端 5）；本輪本機 commit 已建立（請以 `git log -1` 取得當前 hash），未 push、未部署。規格檔 `docs/STUDENT_UI_REDESIGN_1.3.0.md` 是使用者未追蹤檔案，不納入提交；下一步由教師部署 Functions 與推送 Pages。
