...
2026-09-18（第七版規格確認／等待資料結構授權）：主規格第一行已確認「第七版」、補充規格第一行已確認「第二版」，兩檔均存在但為使用者未追蹤檔案，不納入提交。規格要求兩項資料結構變更：① `Progress.wrong` 由舊版 `Record<version, string[]>` 轉為 `Record<version, Record<questionId, { n, at }>>`（舊陣列讀取視為 `at:0`，下次作答寫入新結構）；② 發布新題庫時新增私有 `banks/{...}/grading` 答案表，既有已發布題庫缺檔時 fallback 讀 chunk。使用者本輪指示「需要改資料結構或影響既有已發布題庫時先回報」，因此尚未實作，等待明確確認後才開始。
