...
25.（2026-09-16，1.3.11）專案根目錄的「`教材工作室.command`」是給教材工作室用的雙擊啟動器（純 shell script，`cd` 到自己所在資料夾再跑 `npm run materials:studio`），跟直接下指令行為完全一樣，純粹省打字；不要把它跟正式部署或 CI 流程混在一起，也不要假設使用者電腦上有其他啟動器（例如 macOS App 或 Automator）——目前只有這一個 `.command` 檔案。
