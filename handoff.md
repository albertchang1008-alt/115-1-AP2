...
- 一般分類完整測驗、完成度、綜合練習行為保持不變；複習考前後端皆排除於綜合練習。教師端可新增／修改組卷／刪除；學生端只提供開始作答與錯題閃卡。題庫同步保留複習考、不允許 Sheet 同名覆蓋。
- 驗證完成：`npm run check`（前端 88、Functions 14）及 `git diff --check` 通過。本機提交 `369a5d8 feat: 新增 1.5.0 複習考作業 [Codex]`；未 push、未部署。已知限制依規格：每輪題目不同、提高門檻不回溯既有 passedAt、作答中斷需重新開始。
- 待辦：Claude 驗收複習考組卷、後端配額防線、教師／學生流程；驗收前勿 push、合併或部署。

### 交給 Codex：1.5.0 複習考（作業）（2026-09-22，教師核定、Claude 撰寫規格）

**規格：`docs/REVIEW_EXAM_1.5.0.md`（已核定，請完整照做，不要自行擴充範圍）。**

- 分支：從 `feature/1.4.0-unit-model` 目前 HEAD 開 `feature/1.5.0-review-exam`。
- 重點：複習考＝教師選多個題目分類，後端把題目複製成一個新 `Unit` 的題目池（`buildReviewExam`＋既有 `publishBank`）；每次作答依比例隨機抽 N 題（未考過優先），抽滿且分配正確才算完整作答；最高分 ≥ 門檻即完成；逾期達標仍算完成但標記「逾期完成」（新增 `progress.units[id].passedAt`）；複習考不列入綜合練習；完成度公式維持 v3。
- 一般分類的交卷、完成度、綜合練習行為**不得改變**，既有測試須原樣通過。
- 建議 2 個 commit：①model＋後端＋測試 ②教師後台＋學生端；每個 commit 前 `npm run check`、`git diff --check`。
- 版本 1.5.0（`npm run version:sync`），更新 `DEVELOPMENT_LOG.md`、本檔固定決策與狀態、`docs/UNIT_MODEL_1.4.0.md` 加註。
- **不 push、不部署**；完成後在本檔標示「1.5.0 複習考待 Claude 驗收」。
- 本次廢案（不要做）：一般單元內的「題目練習活動」；以「連到來源分類」方式實作複習單元（規格討論中的舊方案）。
...
### 教材改善任務 1、2 完成；任務 3 擴及三份教材，待教師核定（2026-09-22，Codex）

**已直接實作的任務 1、2：**
- 任務 1：`html/紅血球的恆定機制.html` 的「紅血球恆定全流程」於 ≤760px 改顯示獨立直式 SVG（360px 視窗寬內完整呈現）。缺氧 → 腎臟 EPO → 紅骨髓 → 紅血球／循環 → 網狀內皮系統、globin／胺基酸、heme／鐵、heme／膽紅素及綠色負回饋皆保留；窄版無水平捲動、文字最小 13px。桌機橫式圖保持不變。
- 任務 2：`html/心臟構造.html`、`html/心臟血液供應.html`、`html/紅血球的恆定機制.html` 的第一關，在每次開啟一題時以既有 `shuffleArray` 洗牌選項。正確性採 `{text, isCorrect}`，不再靠原始選項位置；題目順序、11 個題目 ID、6 個節點 ID、首次作答送出方式與通關條件不變。

**任務 3A：三份教材的視覺稿重構（教師指定列入任務 3，未實作）：**
- 共同版型：將每份情境實驗室重構為「四張有醫學插圖主角的因果流程卡」＋清楚水平箭頭；桌機四欄、手機直式堆疊。保留目前滑桿／切換器、狀態文字與預測題，不改診斷事件或分母。若採影像素材，使用風格一致、可在教材內發佈的教學插圖，不把預覽圖直接嵌入。
- `心臟構造.html`：靜脈回流／心房充盈 → 房室瓣開啟 → 心室收縮、房室瓣關閉 → 半月瓣開啟、血液射入動脈；每卡以心腔或瓣膜剖面作視覺主角。
- `心臟血液供應.html`：心室收縮、心肌壓迫冠狀血管 → 冠狀灌流下降 → 心室舒張、心肌放鬆 → 冠狀灌流恢復；每卡以心肌與冠狀血管的截面／血流方向作視覺主角。
- `紅血球的恆定機制.html`：以教師提供的視覺稿作方向：組織缺氧 → 腎臟分泌 EPO → 紅骨髓造血 → 紅血球增加、攜氧恢復；保留負回饋說明並與目前的完整恆定圖相連。這是先前展示的目標視覺，**目前尚未做成該樣式**。

**任務 3B：第二關重試鑑別度（未實作，待教師選擇）：**
- A. 答錯先只顯示提示方向，不立即指出正確選項；五題作答完後才顯示完整答案與解析。優點是最小改動且降低背位置通關；缺點是即時矯正較慢。題目 ID 與 `questionTotal: 11` 不變。
- B. 每份擴為 10 題病例庫、每輪抽 5 題。優點是鑑別度最高；缺點是新增題目 ID、需更新 `shared/materials.ts` 的題目總數與教材工作室欄位，且新／舊診斷資料的 `questionTotal` 會不同，需另定資料解讀規則。若新增題目仍送 `CourseLearning.answer`，正式分母不再是 11，不能直接沿用目前設定。
- C. 答錯後提示必須回到相應探索節點閱讀，完成指定停留／探索再開放重試。優點是促進回讀且不改題庫或分母；缺點是實作需明確定義離線與無障礙替代流程，鑑別提升有限。
- **建議：先採 A＋C。** 兩者可保留 6 節點／11 題、既有 ID 與歷史資料可比性；若仍需要更高的題目鑑別度，再以獨立版本規劃 B。
...
- 驗證完成：內嵌 JS 語法檢查、`npm test` 85 項、`npm run build`、`git diff --check` 通過。提交 `4b88abe feat: 重構三份教材情境流程與回讀重試 [Codex]`；不 push、不部署、不匯入教材工作室。CUA 仍受 `file://` Browser URL policy 阻擋，未能完成人工 390px／桌機畫面巡覽。**任務 3 實作待 Claude 驗收。**
...
**三份新教材（bb50d6f、cd0fc64）**：抽查通過，可進教材工作室匯入。確認項目：三份都載入 course-learning.js；各 6 個探索節點（`<slug>-node-01..06`，點流程卡才送 explore，每節點一次）；11 題 ID 固定（`<slug>-foundation-q01..06`、`<slug>-case-q01..05`）；第一關須逐題答對才能前進、第二關 5 題全對才 `complete()`；預測題與滑桿不送診斷事件；HbA α₂β₂／4 heme／4 O₂、CO₂ 70/23/7、RBC 壽命 120 天等數值正確。節點 ID 由迴圈組出，`materials:audit` 偵測不到節點數屬預期，分母請手填 6／11。
待改善（非阻擋）：①紅血球「恆定全流程」圖 `min-width:720px`，手機上要在框內左右滑，建議 ≤760px 改直式；②第一關選項沒有洗牌（第二關有）；③第二關答錯即顯示正解，重試同 5 題容易靠記憶通關——首次診斷不受影響，但「通關」的鑑別度低，是否改題庫輪替由教師決定。
流程備註：教師原要求「先提計畫、核定後才實作」，handoff 記載為「依教師確認的視覺語言」實作；Claude 無法從紀錄確認教師已核定，已請教師確認。

### 紅血球 HbA 構造、恆定統整圖與窄版流程排版（2026-09-21，Codex）：已完成，待 push／待教材工作室正式匯入

- `html/紅血球的恆定機制.html` 的 Hb 圖已依成人主要 HbA 的 `α₂β₂` 四聚體重畫：2 個 α、2 個 β 珠蛋白次單元；每個次單元各有 1 個含 Fe²⁺ 的 heme，合計最多可逆結合 4 個 O₂。CO₂ 運輸比例拆為三個獨立標籤，避免單行文字擠壓。
- 紅血球情境實驗室新增「紅血球恆定全流程」：缺氧 → 腎臟感氧細胞／EPO → 紅骨髓 → 紅血球與循環 → 網狀內皮系統，並標示 globin／胺基酸、heme／鐵回收、膽紅素排出及綠色負回饋；既有 6 個正式探索節點與 11 題診斷分母不變。
...
依教師確認的視覺語言，完成未追蹤原始檔 `html/心臟構造.html`、`html/心臟血液供應.html`、`html/紅血球的恆定機制.html`：
- 心臟構造：舒張／收縮切換，呈現房室瓣關閉與半月瓣開啟的單向血流。
- 心臟血液供應：收縮／舒張切換，呈現心肌壓迫血管與舒張期冠狀灌流。
- 紅血球恆定：缺氧程度控制器，呈現組織缺氧→腎臟 EPO→紅骨髓造血→攜氧恢復的負回饋。
...
All 畫面會列出「以下班級另有設定：班級（欄位）」；其欄位與上方單元共用欄位由同一次草稿更新，保持即時一致。新增測試覆蓋 All 共用值、單班優先與恢復清空。`npm run check` 已通過（前端 81、Functions 13）。未改 Functions、Firestore、Google Sheet；未部署、未合併或推送 main。**All 修正待 Claude 驗收（實作 commit `340c39e`）。**

Claude 驗收時請確認：All 修改任一欄位後，上方單元欄位同步更新、仍有不同覆寫的班級提示正確；選單切至單一班級後，該班覆寫仍能單獨設定；All 的恢復共用設定會移除該單元全部班級覆寫。
...
學生回報「我的學習紀錄：尚無作答紀錄」，畫面有待同步提示。根因：教師在單元設定填了開放時間（datetime-local，無時區），Cloud Functions 以 UTC 解讀，開放後 8 小時內 submitAttempt 一律回「單元尚未開放」，作答只存在學生瀏覽器佇列。修正：`shared/model.ts` 新增 `parseCourseTime()`（無時區字串視為 +08:00），伺服器 submitAttempt／submitMixedAttempts／saveActivity／解析研究／saveLearningEvents 與學生端鎖定顯示改用它。另修完成度看板空白（progress 缺 activities 時丟錯）。`npm run check` 前端 76、Functions 13。部署後請學生重新整理或按「重新同步」，佇列作答會補送。**追加（未部署，只需推送前端）：** 學生 張家瑄（護525）有 22 組待同步。每次交卷都先放本機佇列，成功才移除；舊版「重新同步」遇第一筆被拒就停，後面全部卡住。已改為逐筆送出並彙整被拒原因（`src/Student.tsx` sync()），前端 77 項通過。推送：`git -C ~/Documents/ChatGPT/課程平台1.0 push origin feature/1.4.0-unit-model:main`。**注意：時區只解釋開放後 8 小時內的失敗；教師指出同時段其他人可交卷，根因尚未由伺服器紀錄證實。**待辦：請教師執行 `firebase functions:log --only platform:submitAttempt --project ap2-7ed91 -n 50` 取得被拒原因，並確認學生重新同步後紀錄是否出現。
...
**1.4.0 上線執行紀錄（Codex，2026-09-18）**
- 步驟 1 ✅：確認分支 `feature/1.4.0-unit-model`、工作目錄乾淨；上線前完整 check 通過（前端 69/69、Functions 12/12、TypeScript、Functions 入口與 Vite production build 成功）。
- 步驟 2–4：待執行。
...
**教師決定（2026-09-18）：先做精簡版再上線。** 交給 Codex 的 1.4.0 精簡任務（已完成）：
...
### 1.4.0 第五次待 Claude 驗收（Codex 已修正手機排版，2026-09-18）

分支 `feature/1.4.0-unit-model`，本輪手機排版實作 commit `00504e0`（精簡列表實作為 `5dd7f14`）。已依第四次驗收指定的 CSS，在 600px 以下將練習列改為兩欄、四列區域，讓分類／分數、測驗／閃卡、抽題與錯題完整顯示；同時取消橫向捲動，抽題按鈕平分可用寬度。

已新增靜態測試精確確認這組 600px 規則存在；`npm run check` 的等效完整指令已通過：前端 69/69、Functions 12/12，TypeScript、Functions 入口與 Vite production build 均成功。本輪沒有部署、沒有合併或推送 main。**現在停下，交 Claude 第五次驗收。**
...
3. 更新 `DEVELOPMENT_LOG.md`、本檔固定決策 10，commit 結尾附身分標註。
4. **先停在這裡，不要部署、不要合併 main。** 在本檔寫「1.4.0 待 Claude 驗收」並附 commit hash。
5. Claude 驗收通過、教師同意後，才進行：部署 `functions:platform:syncSheet` → 合併 push main → 確認正式站 version.json＝1.4.0。
6. 之後教師在後台：同步題庫 → 按「全部移除」清掉舊結構單元 → 在新單元重新加入活動（例如課程簡介的 HTML 教材）→ 對照表勾班級 → 發布。
...
2026-09-18（教師版面回歸修正）：iPhone 修正最初把教師 prefs-bar 放進 `.app` CSS grid，導致 PDF 所示的側欄與內容欄錯置／空白。已改為偏好列和 `.app` 同層，`.app` 恢復只有 sidebar 與 main 兩個 grid 子項；加入結構測試。`npm run check` 通過（前端 51、Functions 7）。規格原檔仍未追蹤，不可納入提交。
2026-09-18（教師頂部留白修正）：後續截圖顯示同層 prefs-bar 仍佔正常文件流的一列，雖不再破壞 grid，仍讓整個後台下移。已改為右上 fixed 控制，桌機 `topbar` 右側預留 460px，避免與偏好選單重疊；不再佔據 sidebar／workspace 的垂直空間。`npm run check` 通過（前端 51、Functions 7）。規格原檔仍未追蹤，不可納入提交。
2026-09-18（看板整組移動）：學習進度看板每個 `Unit.group` 標題下新增「整個單元／整組移到…」選單；選擇區域會將同 group 的所有次單元 ID 一次傳給既有 `setUnitVisibility` transaction，仍只更新區域與提示欄位。個別次單元的「移到…」保留。新增靜態 UI 測試；`npm run check` 通過（前端 52、Functions 7）。規格原檔仍未追蹤，不可納入提交。
2026-09-18（題庫同步來源入口）：使用者回報找不到貼 Google Sheet 網址／ID 的設定。原設定仍在平台設定，但已直接放回題庫管理頁最上方的「同步檔案設定」面板，開頁讀取 `getSyncStatus` 的既有值、按保存呼叫既有 `saveSheetConfig`，無後端或資料結構變更。同步訊息「找不到與課程代碼『115-1-AP2』同名的題庫分頁」代表檔案設定可以讀取，但該 Google Sheet 內的工作表分頁名稱未與課程代碼逐字相同；應將分頁更名為 `115-1-AP2` 或改用含此分頁的 Sheet。`npm run check` 通過（前端 53、Functions 7）。未 push、未 deploy。
2026-09-18（平台設定側欄可見性）：使用者截圖證實「平台設定」在長側欄導覽後被裁出視窗。已把它移為使用者帳號列上方的固定側欄入口，主導覽清單可自行捲動；高字級、低高度視窗都可直接開啟。保留原本的 Settings 頁與題庫管理頁同步來源入口。新增靜態結構測試；`npm run check` 通過（前端 54、Functions 7）。未 push、未 deploy。
2026-09-18（同步題庫現場診斷與修正）：使用者截圖已證實工作表視覺名稱是 `115-1-AP2`，因此不能再斷言單純改名即可。既有前端訊息是以 `syncSheet` 回傳的 `banks` 是否含目前課程判斷，無法分辨「設定到另一份同名 Sheet」、「分頁前後隱藏空白」與「表內舊課程代碼欄指到別課」。已修正同步：只忽略 Sheets 分頁名稱前後空白、回傳 `availableTabs` 讓前端在未匹配時列出 API 實際讀到的 JSON 引號名稱；每筆結果帶 `sourceTab/sourceCourseId`，且表內若有課程代碼與分頁課程不一致會在寫入前拒絕。新增前端診斷與後端跨課程防護測試；`npm run check` 通過（前端 55、Functions 8）。此修正含 Cloud Functions，**目前僅本機 commit，需日後明確部署 Functions 與發布前端後，才能在該畫面顯示實際清單並進一步定位。**未 push、未 deploy。
2026-09-18（部署落差已確認）：使用者重新操作後，toast 顯示「未提供分頁清單，請確認雲端 Functions 已更新」。此文字只會在新版前端沒有收到 `availableTabs` 時出現，已證實目前頁面使用新版前端／本機程式，但 callable `syncSheet` 仍是舊版已部署 Cloud Function。下一步須獲使用者明確授權後執行 `firebase deploy --only functions:syncSheet`；部署後再次同步即會列出 Google API 實際讀到的分頁名稱，才能判定真正的 Sheet／字元／資料欄位落差。尚未部署、未 push。
2026-09-18（syncSheet 部署嘗試，狀態未證實）：使用者已授權僅部署此 Function。已確認專案有 `platform` codebase，正確目標為 `functions:platform:syncSheet`；第一次缺少 codebase 前綴被 CLI 安全拒絕，沒有改動。之後三次精確部署均完成 predeploy build、Functions source upload，輸出停在 `updating Node.js 22 (2nd Gen) function platform:syncSheet(asia-east1)...`，但 CLI 沒有輸出成功或失敗結尾，工具也未提供可讀退出碼；`firebase functions:list` 只能確認 endpoint 存在、不能確認 revision，本機也無 `gcloud`。為避免重複併發更新，已停止重試。需待 Firebase 背景更新完成後，由教師重新按一次同步題庫：若仍見「未提供分頁清單」，代表未完成；若顯示實際 tabs，請回傳該訊息以做下一步精確處理。未 push、未發布 Pages。
2026-09-18（同步已讀取題庫但錯誤訊息不足）：使用者下一次同步已顯示現有 5 題、toast 改為「題庫同步有未完成項目」，表示分頁比對已通過且至少一個次單元已開始同步；剩下是某一筆 `banks` error，但前端將細節吞掉。已將 toast 改為顯示前三筆 `次單元／分頁：後端錯誤`，不需再更新 Functions。新增前端測試，`npm run check` 通過（前端 56、Functions 8）。目前本機 commit，未 push、未發布 Pages。
2026-09-18（正式 Sheet 第 2 列解答相容）：依使用者提供公開 Sheet 的唯讀資料端點，確認 `115-1-AP2` 第 2 列的「正確答案文字」為空、`原始答案字母(僅對照)` 為 `B`，選項 B 為正解；這正是錯誤「解答無法唯一對應選項」的原因。已改 `parseBankSheet`：文字答案空白時才 fallback 使用原始答案字母，文字答案有值時不變；新增精確單元測試。`npm run check` 通過（前端 57、Functions 8）。沿用授權只部署 `functions:platform:syncSheet`，Firebase 完成 source upload 後仍停在 update started 而無成功／失敗結尾，狀態尚未由 CLI 證實；請稍候再同步驗證。未 push、未發布 Pages。
2026-09-18（待教師決策：答案欄位治理）：教師提出是否需要「正確答案文字」與「原始答案字母(僅對照)」兩欄。平台內部唯一需要的是選項代碼 `a`–`d`；建議長期僅維護一欄正式的「正確答案代碼」（A–D），文字答案由選項推得，不讓兩欄人工同步。若保留「原始答案字母」，僅作從舊系統匯入的稽核／追溯欄，不可參與正式批改；現有 fallback 是相容措施，待教師選定最終欄位契約後再調整，尚未因這次討論改 Sheet 或程式。
2026-09-18（答案欄位統一，待教師改 Sheet）：教師決定不再維護兩個正式答案欄，且會自行修改 Google Sheet。平台已改為**只接受**「正確答案代碼」（A–H 或 1–8）作題庫正式答案，拒絕以選項文字當答案；「Zuvio解答」是教師另行出題／匯出欄，匯入器刻意不讀取、不保存、不進資料庫。題庫範本新增兩欄並相鄰，`FillCourseAndUnit.gs`、題庫文件與前後端測試已同步。`npm run check` 通過（前端 57、Functions 8）。**暫不部署本版 `syncSheet`**：教師須先把 `115-1-AP2` 的「正確答案文字」改為「正確答案代碼」、各列填 A–D，並新增相鄰的「Zuvio解答」後，再明確指示部署；否則現行 Sheet 會因缺少正式答案欄被拒絕。Google Sheet 本輪未被 agent 寫入，未 push、未發布 Pages。
...
40.（2026-09-19）NotebookLM 新版資訊圖表提示詞應以目前教材格式為基準：六個固定節點與 SVG 教學圖、卡片內採圖→概念→重點→臨床意義、第一關 6 題先備知識逐題解鎖、第二關 5 題隨機病例題全對通關。NotebookLM 初稿不得自行加入追蹤、GA4、Firebase 或個資；平台串接與正式驗收由後續修飾階段處理。
