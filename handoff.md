...
後端 Functions：**1.2.4 已部署且確認生效**（使用者實測「同步班級名冊」看到具體的「名冊格式錯誤：...」訊息，取代了原本的 internal/500，證實修正有效）。**1.2.5（移除信箱網域限制）使用者已於 2026-09-16 回報部署完成**——這個沙箱沒有 Firebase 專案存取權限，沒辦法直接查 Functions 版本或呼叫紀錄來交叉驗證，是採信使用者的回報；**尚未確認的是部署後有沒有實際重新按過「同步班級名冊」看行為是否如預期**（見下方「下一步需要的外部輸入」），這件事還在等使用者回報。
...
- **實測確認（1.2.5，後端，使用者已回報 2026-09-16 部署完成，還缺這一步）**：
  麻煩在「班級名冊」頁按一次「同步班級名冊」。信箱網域限制已經拿掉，如果
  還是失敗，畫面應該會指出是第幾列、哪一類欄位問題（學號格式、班級代碼
  格式、姓名空白等），麻煩把訊息內容回報，才能判斷下一步；如果成功了，
  也麻煩說一聲，這樣才能把這項待辦徹底關掉。
