...
- 三份新教材視覺：教師不滿意目前的 AI 插圖流程卡。Claude 建議方向：①先定視覺規範（配色、字級≥14px、元件）②改「一張 SVG 圖多狀態」（如心臟瓣膜開合）取代四張相似插圖 ③插圖改用有授權的一致圖庫（如 Servier Medical Art，CC BY）④知識節點小圖放大重做 ⑤先以「心臟構造」做樣板，核定後再推其他份。尚待教師選擇是否先做原型。
- 長期流程（Claude 提案，待核定）：教材改「共用元件庫＋每份一個內容檔＋一鍵產生頁面」、SVG 醫學圖庫、自動截圖驗收指令、規格決策清單、把流程寫成 Skill、`App.tsx` 拆檔。
