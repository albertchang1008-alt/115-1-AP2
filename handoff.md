...
2026-09-18（題庫欄位統一驗證）：教師已自行調整正式 Google Sheet，並以唯讀方式確認 `115-1-AP2` 分頁的 M 欄為「正確答案代碼」，前兩題值皆為 `B`；相鄰 N 欄「Zuvio解答」存在，但平台依既定契約完全忽略、不會寫入資料庫。目前程式契約已在本機 commit `cf9f8ea`，`npm run check` 已通過（前端 57、Functions 8），未 push。曾嘗試只部署 `functions:platform:syncSheet`，但因先前「不要 firebase deploy」限制仍有效而被安全系統拒絕；需使用者再次明確授權 Firebase 正式環境部署後才能發布此 callable。Google Sheet 本輪未由 agent 寫入。
