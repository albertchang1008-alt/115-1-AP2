...
### 1.4.0 第四次待 Claude 驗收（Codex 已完成精簡版，2026-09-18）

分支 `feature/1.4.0-unit-model`，本輪精簡版實作 commit `5dd7f14`。學生單元的練習分頁已改為每個題目分類一列，同列顯示最高分／門檻、綠色「已達標」、完整測驗、完整閃卡、10／20／30 題抽題與錯題數；無錯題時錯題按鈕停用。入口沿用原本 `go()/start()` 與路由，未改計分或完成度規則。教師頁副標已改「單元設定」，班級對照表改顯示「全部（N 個分類）」。

精簡列表、四種入口、錯題停用狀態與兩處文案都有對應測試；`npm run check` 的等效完整指令已通過：前端 69/69、Functions 12/12，TypeScript、Functions 入口與 Vite production build 均成功。本輪沒有部署、沒有合併或推送 main。**現在停下，交 Claude 第四次驗收。**
