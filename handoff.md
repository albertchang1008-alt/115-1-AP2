...
2026-09-18（同步題庫現場診斷）：使用者畫面顯示 Sheet ID 已保存，且同步回傳「找不到與課程代碼 `115-1-AP2` 同名的題庫分頁」。已查證此訊息只有在後端成功讀取 Sheets metadata、卻沒有 title 與受管課程 ID 完全相同時才出現；不是 ID 格式、服務帳戶權限或題庫欄位錯誤。教師需將該試算表底部工作表分頁名稱改為精確的 `115-1-AP2`（半形連字號、沒有前後空白）再同步；或將課程代碼改成既有分頁名稱（課程代碼建立後不可變更，通常不建議）。未改同步規則、未 push、未 deploy。
