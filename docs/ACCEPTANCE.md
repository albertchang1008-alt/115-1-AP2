...
## 1.2.0 接手驗證（2026-09-14，本機）

- Node 22.23.2 / darwin arm64，未重新安裝套件。`npm run check` 通過。
- 26 項前端／共用邏輯測試、TypeScript／Vite 前端建置、Functions 建置與 29 個入口載入通過。
- 1 項實際 callable＋記憶體資料庫整合測試通過：空白題序的 500 題發布、題庫重排序不升版、500 題完整交卷、重送不重複計分、501 題拒絕。
- 不包含 Emulator、真實 Sheet 或線上教學驗收；沒有 push、合併或部署。
