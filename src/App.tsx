...
function ChapterOverrides({ course, name, chapter, onChange, changeShared }: { course: Course; name: string; chapter: Chapter; onChange: (v: Course['chapterOverrides']) => void; changeShared: (patch: Partial<Chapter>, overrides: Course['chapterOverrides']) => void }) {
  const ALL_CLASSES = '__all__';
  const [cl, setCl] = useState(ALL_CLASSES);
  const all = cl === ALL_CLASSES;
  const selectedClasses = cl === ALL_CLASSES ? course.classIds : [cl];
  const value = all ? chapter : course.chapterOverrides?.[cl]?.[name] || {};
  const clear = (fields?: string[]) => { const next = structuredClone(course.chapterOverrides || {}); selectedClasses.forEach((id) => { const override = next[id]?.[name]; if (!override) return; if (fields) fields.forEach((field) => delete (override as any)[field]); else delete next[id][name]; if (next[id]?.[name] && !Object.keys(next[id][name]).length) delete next[id][name]; if (next[id] && !Object.keys(next[id]).length) delete next[id]; }); return next; };
  const change = (patch: Partial<Chapter>) => all ? changeShared(patch, clear(Object.keys(patch))) : onChange({ ...course.chapterOverrides, [cl]: { ...course.chapterOverrides?.[cl], [name]: { ...value, ...patch } } });
  const restore = () => onChange(clear());
  const differing = course.classIds.flatMap((id) => { const override = course.chapterOverrides?.[id]?.[name]; const fields = [['threshold', '門檻'], ['opensAt', '開放時間'], ['dueAt', '完成期限'], ['required', '必做']].filter(([field]) => override && (override as any)[field] !== undefined).map(([, label]) => label); return fields.length ? `${course.classNames?.[id] || id}（${fields.join('、')}）` : []; });
  return <details><summary>各班級的門檻與開放安排</summary><p className="muted">未設定時沿用單元共用值，教材內容保持共用。選擇 All 後，修改的是單元共用設定。</p>
    ...
    {all && differing.length > 0 && <p className="notice">以下班級另有設定：{differing.join('、')}</p>}
    {cl && <div className="formgrid"><Field label={all ? '單元共用達標分數' : '此班達標分數'}><input type="number" min="0" max="100" value={value.threshold ?? chapter.threshold} onChange={(e) => change({ threshold: Number(e.target.value) })} /></Field><Field label={all ? '單元共用開放時間' : '此班開放時間'}><input type="datetime-local" value={value.opensAt ?? chapter.opensAt} onChange={(e) => change({ opensAt: e.target.value })} /></Field><Field label={all ? '單元共用完成期限' : '此班完成期限'}><input type="datetime-local" value={value.dueAt ?? chapter.dueAt} onChange={(e) => change({ dueAt: e.target.value })} /></Field><label className="check"><input type="checkbox" checked={value.required ?? chapter.required} onChange={(e) => change({ required: e.target.checked })} />{all ? '單元列為必做' : '此班列為必做'}</label></div>}
...
function CourseEditor(props: CourseEditorProps) {
  ...
  const changeChapterAndOverrides = (patch: Partial<Chapter>, chapterOverrides: Course['chapterOverrides']) => { const base = materialize(); setDraft({ ...base, chapterOverrides, chapters: { ...base.chapters, [name]: { ...chaptersOf(base)[name], ...patch } } }); };
  ...
  const buildReview = async (input?: { name: string; sources: string[]; count: number }) => { const examName = input?.name || reviewName, sources = input?.sources || reviewSources, count = input?.count || reviewCount, previous = draft.units.find((u) => u.id === examName); if (previous?.review && !confirm('會產生新的考卷版本。學生已考到的最高分保留；舊版本的錯題不帶到新版本。')) return; setBusy(true); try { const r = await api.call<any>('buildReviewExam', { courseId: draft.id, name: examName, sourceUnitIds: sources, drawCount: count }); const review = { sourceUnitIds: sources, sourceVersions: r.sourceVersions, drawCount: count, allocation: r.allocation, builtAt: r.builtAt, history: [{ version: r.version, drawCount: count, allocation: r.allocation, builtAt: r.builtAt }, ...(previous?.review?.history || [])].slice(0, 10) }; const u: Unit = { id: examName, title: examName, group: examName, description: '', required: true, threshold: 80, opensAt: '', dueAt: '', bankVersion: r.version, questionCount: count, activities: [], visibility: 'hidden', review }; const base = materialize(); const exists = base.units.some((x) => x.id === examName); setDraft({ ...base, units: exists ? base.units.map((x) => x.id === examName ? { ...x, bankVersion: r.version, questionCount: count, review } : x) : [...base.units, u], chapters: exists ? base.chapters : { ...base.chapters, [examName]: { title: examName, description: '', required: true, threshold: 80, opensAt: '', dueAt: '', activities: [] } }, chapterOrder: exists ? base.chapterOrder : [...(base.chapterOrder || []), examName] }); setSelected(examName); setReviewOpen(false); notify('複習考題目池已建立；請設定開放與適用班級後保存草稿。'); } catch (e) { notify((e as Error).message); } finally { setBusy(false); } };
          ...
          onClick={() =>
            download(
              ...
              filtered.map((r) => {
                ...
                const overdue = forClass(publishedCourse, cl).units.filter((u) => isReviewUnit(u) && isOverdue(u, r.progress.units[u.id])).map((u) => u.title).join('、');
                return {
                  姓名: r.name,
                  學號: r.studentId,
                  班級: cl,
                  ...
                  <tr key={r.email}>
                    ...
                    <td>
                      {forClass(publishedCourse, cl)
                        ...
                        .map((u) => u.title)
                        .join('、') || '—'}{forClass(publishedCourse, cl).units.filter((u) => isReviewUnit(u) && isOverdue(u, p.units[u.id])).map((u) => <small className="block" key={u.id}>逾期完成：{u.title}</small>)}
                    </td>
                    <td>
